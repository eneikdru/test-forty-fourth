...
public class SearchService {
    ...
    public SearchResponse search(Connection conn, String q, String category, String status, String sort, int page, int limit) throws SQLException {
        ...
        if (q != null && !q.trim().isEmpty()) {
            if (isPostgreSQL) {
                whereClause.append("to_tsvector('english', coalesce(title, '') || ' ' || coalesce(content, '')) @@ plainto_tsquery('english', ?)");
        ...
        long total = 0;
        String countSql = "SELECT COUNT(*) FROM materials" + whereSql;
        ...
        String selectSql = "SELECT id, title, content, category, status, updated_at FROM materials" +
                whereSql +
